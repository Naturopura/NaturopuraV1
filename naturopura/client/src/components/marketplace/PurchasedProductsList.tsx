...
// Helper function to format amount as ETH
const formatAmountAsEth = (amount: number) => {
...
const PurchasedProductsList = ({ userRole }: PurchasedProductsListProps) => {
  ...
  useEffect(() => {
    const fetchPurchases = async () => {
      try {
        ...
        if (!role) {
          ...
          if (userString) {
            try {
              ...
              role = userObj.role as 'admin' | 'delivery_partner' | 'farmer' || undefined;
            } catch {
              role = undefined; // <-- fix here
            }
          }
        }
        let response;
        ...
        setError('Failed to fetch purchases: ' + (error.response?.data?.message || error.message || 'Unknown error'));
        toast({
          title: "Error",
          description: "Failed to fetch purchased products: " + (error.response?.data?.message || error.message || 'Unknown error'),
          variant: "destructive",
        });
        setLoading(false);
      }
  ...
  useEffect(() => {
    ...
    if (searchTerm) {
      filtered = filtered.filter(purchase => {
        ...
        const userEmail = isUserObject(purchase.userId) ? purchase.userId.email.toLowerCase() : '';
  ...
  function isUserObject(userId: any): userId is { _id: string; email: string } {
    return userId && typeof userId === 'object' && '_id' in userId && 'email' in userId;
  }
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center h-96 space-y-4">
        ...
        <p className="text-gray-600 font-medium">Loading purchased products...</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="text-center">
        ...
        <h2 className="text-3xl font-bold text-gray-900 mb-2">Purchased Products</h2>
        <p className="text-gray-600 max-w-2xl mx-auto">
          Track and manage all purchased products with detailed information and real-time status updates.
        </p>
      </div>
        ...
        <div className="bg-gradient-to-r from-blue-50 to-blue-100 rounded-2xl p-6 border border-blue-200">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-blue-700">{purchases.length}</p>
            </div>
            ...
            return (
              ...
              >
                <div className="flex items-start justify-between mb-4">
                  ...
                  <div className={`inline-flex items-center gap-1 px-3 py-1 rounded-full text-xs font-medium border ${getStatusColor(purchase.status)}`}>
                ...
                <div className="space-y-3 mb-6">
                  <div className="flex items-center gap-3 text-sm text-gray-600">
                    ...
                    <span className="truncate">
                      {isUserObject(purchase.userId) ? purchase.userId.email : purchase.userId}
                    </span>
                  </div>
                  ...
                  <div className="flex items-center gap-3 text-sm">
                    ...
                    <span className="font-semibold text-gray-900">
                      {formatEther(parseEther(String(purchase.amount)))} ETH
                    </span>
                  </div>
                ...
                <Button 
                  ...
                  className="w-full bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 text-white font-semibold py-3 rounded-xl transition-all duration-200 shadow-md hover:shadow-lg"
                >
          ...
          <div className="bg-white rounded-3xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            ...
            <div className="p-6 space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                  <div>
                    <p className="font-semibold text-gray-900 text-lg mb-2">
                      {products.find(p => p._id === selectedPurchase.productId)?.name || 'Unknown Product'}
                    </p>
                    <p className="text-gray-600 mb-3">
                      {products.find(p => p._id === selectedPurchase.productId)?.description}
                    </p>
                  </div>
                  <div className="space-y-2">
                    <div><span className="font-medium text-gray-700">Category:</span> {products.find(p => p._id === selectedPurchase.productId)?.category}</div>
                    <div><span className="font-medium text-gray-700">Unit:</span> {products.find(p => p._id === selectedPurchase.productId)?.unit}</div>
                    <div><span className="font-medium text-gray-700">Quantity:</span> {products.find(p => p._id === selectedPurchase.productId)?.quantity}</div>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-gradient-to-r from-green-50 to-emerald-50 rounded-2xl p-6 border border-green-200">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                  <div>
                    <div className="mb-2"><span className="font-medium text-gray-700">Amount:</span> <span className="font-bold text-emerald-700">{formatAmountAsEth(selectedPurchase.amount)} ETH</span></div>
                    <div className="mb-2"><span className="font-medium text-gray-700">Payment Method:</span> <span className="capitalize">{selectedPurchase.paymentMethod}</span></div>
                  </div>
                  <div>
              ...
              <div className="bg-gradient-to-r from-blue-50 to-indigo-50 rounded-2xl p-6 border border-blue-200">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                  <div>
                    <div className="mb-2"><span className="font-medium text-gray-700">Address:</span></div>
                    <div className="p-3 bg-white/80 rounded-lg text-gray-900">{selectedPurchase.address}</div>
                  </div>
                  <div>
                    <div className="mb-2"><span className="font-medium text-gray-700">Pincode:</span> {selectedPurchase.pincode}</div>
                    <div className="mb-2"><span className="font-medium text-gray-700">Buyer:</span> {isUserObject(selectedPurchase.userId) ? selectedPurchase.userId.email : selectedPurchase.userId}</div>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-gray-50 rounded-2xl p-6 border border-gray-200">
                ...
                <div className="text-sm text-gray-600">
                  <div className="flex items-center gap-3">
                    ...
                    <span>Order placed on {new Date(selectedPurchase.createdAt).toLocaleString()}</span>
                  </div>
                </div>
              </div>
            </div>
