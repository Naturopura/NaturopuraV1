...
const AdminSidebar = () => {
  ...
  useEffect(() => {
    ...
    const menuToExpand = menuItems
      .filter((item) =>
        item.submenu?.some((subItem) => currentPath.startsWith(subItem.path))
      )
      .map((item) => item.title);
    ...
    // Only close sidebar on mobile if it's currently open and screen becomes mobile
    if (isMobile) {
      // For AdminFeedback and FarmerList pages, keep sidebar open on mobile for better compatibility
      if (
        currentPath.startsWith("/admin/feedback") ||
        currentPath.startsWith("/admin/farmers")
      ) {
        // Instead of forcing open, toggle sidebar to open if closed
        if (!isExpanded) {
          toggleSidebar(true);
        }
      } else {
        toggleSidebar(false); // Close sidebar on mobile by default or when resized to mobile
      }
    } else {
                          ...
                          <div
                            className={cn(
                              ...
                              location.pathname.startsWith(subItem.path)
                                ? "bg-green-50 naturopura-text font-medium"
