  ...
  return (
    <FarmerLayout title="Cold Store" subtitle="Cold Store Tracker">
      <div className="max-w-3xl mx-auto mt-10 p-6 bg-white/80 rounded-3xl shadow-2xl border border-emerald-200">
        <h2 className="text-2xl font-bold naturopura-text mb-6 flex items-center">
          <svg className="w-7 h-7 mr-3 naturopura-text" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div className="flex gap-5 overflow-x-auto pb-2">
          {coldStores.map((store) => {
            ...
            return (
              ...
              >
                <div>
                  <div className="text-lg font-semibold naturopura-text">{store.name}</div>
                  <div className="text-sm text-black">{store.vicinity}</div>
                  {store.rating && (
                    <div className="text-xs text-black">Rating: {store.rating} ⭐</div>
                  )}
                  {capacityInfo ? (
                    ...
                    <div className="text-xs text-black mt-1 italic">Capacity info not available</div>
                  )}
                </div>
