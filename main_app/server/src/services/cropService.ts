...
import { GeminiResponse, HealthIssueInfo, MultiLangString, MultiLangStringArray } from "../types/cropTypes";
...
const plantHealthDatabase: Record<string, { diseases: { name: string; symptoms: MultiLangString; treatment: MultiLangStringArray; causes: MultiLangStringArray }[] }> = {
  tomato: {
    diseases: [
      {
...
export const detectCropIssueService = async (fileBuffer: Buffer, mimeType: string): Promise<GeminiResponse> => {
  ...
  let geminiParsedResult: GeminiResponse;
  try {
    ...
    const defaultMultiLangString: MultiLangString = { en: "N/A", hi: "लागू नहीं", or: "ଲାଗୁ ନୁହେଁ" };
    const defaultMultiLangArray: MultiLangStringArray = { en: [], hi: [], or: [] };
  ...
  const plantNameEn = geminiParsedResult.plant_name?.en?.toLowerCase(); // Use English for matching
  const issueNameEn = geminiParsedResult.issue_info?.name?.en?.toLowerCase(); // Use English for matching
  ...
  if (geminiParsedResult.issue_detected && plantNameEn && issueNameEn) {
    ...
    if (localPlantData) {
      const matchedDisease = localPlantData.diseases.find(d =>
        issueNameEn.includes(d.name.toLowerCase())
      );
