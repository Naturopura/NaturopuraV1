...
export const detectCropHealth = async (
  ...
  try {
    ...
    if (!imageFile) {
      res.status(STATUS_CODE.BAD_REQUEST).json({ error: "No image provided", details: "No image provided" });
      return;
    }
    ...
    res.status(STATUS_CODE.INTERNAL_SERVER_ERROR).json({ error: "Failed to detect crop health", details: error.message });
  }
};
