...
import contractABI from "../../constants/naturopuraPayment.json";
...
export interface MetaMaskPaymentFormProps {
  productId: string;
  onSuccess: () => void;
...
const MetaMaskPaymentForm = ({
  ...
  const [selectedNetwork, setSelectedNetwork] = useState(
    () => SUPPORTED_CURRENCIES[0].networks[0]
  );
    ...
    const fetchPrice = async () => {
      try {
        ...
        const response = await fetch(
          `${API_BASE_URL}${ENDPOINTS.GET_PRODUCT(productId)}`,
          {
            headers: { Authorization: `Bearer ${token}` },
          }
        );
        if (!response.ok)
          throw new Error("Failed to fetch product price from backend");
        ...
        const errMsg =
          error instanceof Error
            ? error.message
            : "Failed to fetch product price from backend.";
        toast({
  ...
  const handlePayment = async () => {
    if (!isConnected || !account) {
      toast({
        ...
        description: "Connect your wallet to proceed with the payment.",
        variant: "destructive",
      });
      return;
    }
    if (
      ...
      toast({
        ...
        description: "Please enter your full shipping address.",
        variant: "destructive",
      });
      return;
    }
    if (!convertedPrice) {
      toast({
        ...
        description: "Product price is not available. Please try again later.",
        variant: "destructive",
      });
      return;
    }
    if (!selectedNetwork.contractAddress) {
      toast({
        title: "Network Error",
        description: "No contract address for selected network.",
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      await switchNetwork(
        "0x" + parseInt(selectedNetwork.chainId).toString(16)
      );
      ...
      const tx = await contract.depositNative({ value: priceWei });
  ...
  const handleNetworkSelect = (networkName: string) => {
    const network = selectedCurrency.networks.find(
        ...
        <CardContent className="p-6">
          <div className="space-y-4">
            ...
            <Button
              ...
              disabled={
                disabled || isProcessing || !convertedPrice || !isConnected
              }
            ...
            >
              {isProcessing ? (
                ...
                <div className="flex items-center space-x-2">
                  ...
                  <span>
                    {convertedPrice
                      ? `Pay ${convertedPrice} ${selectedCurrency.symbol} with MetaMask`
                      : "Loading..."}
                  </span>
                </div>
              )}
